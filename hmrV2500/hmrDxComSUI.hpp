		...
		*/
		...
		virtual int extend_draw(dxO& dxo)override{
			...
			try{
				...
				dxo.draw(Pint(115, 55),dxoStrP(Pint(105,20),hmr::time_to_hms(SendTimeInq()),getClr(normal,strobj)));
			}catch(const hmLib::inquiries::unconnected_exception&){				
			...
			try{
				...
				dxo.draw(Pint(115, 80),dxoStrP(Pint(105,20),hmr::time_to_hms(RecvTimeInq()),getClr(normal,strobj)));
			}catch(const hmLib::inquiries::unconnected_exception&){				
