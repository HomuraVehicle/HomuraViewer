#ifndef HMR_HUMID_INC
#define HMR_HUMID_INC 100
#

/*=========hmrHumid===========
hmrHumid v1_01/130602 iwahori
	キーボード・パッド用のslot_setDataMode()を追加(引数がvoid(void)になっている)
hmrHumid v1_00/130511 iwahori
	cHumidMsAgentのsignal-slot,inquiry-contactへの対応完了
...
namespace hmr{
	class cHumidMsgAgent:public itfMessageAgent{
	private:
...
//		static const double D_ADMaxValue;
	public:
		static double toValue(unsigned char LowByte,unsigned char HighByte){
			...
			return static_cast<double>((static_cast<double>(tmp)/D_ADMaxValue*100./2.4/4.));
		}
