	...
	public:
		...
		int normal_draw(dxO& dxo)override{
			...
			try{
				...
				// Interval 設定
				dxRGB intClr;
				if(isSendedFlag){ intClr = getClr(active, butobj);
			...
			try{
				...
				// timeout 設定
				dxRGB timClr;
				if(isSendedFlag){ timClr = getClr(active, butobj);
		...
		int extend_draw(dxO& dxo)override{
			...
			if(isSendedFlag){
				dxo.draw(Pint(115, 80),dxoStrP(Pint(105,20),hmr::time_to_hms(sendedTime),getClr(active,strobj)));
			}else{
				dxo.draw(Pint(115, 80),dxoStrP(Pint(105,20),hmr::time_to_hms(sendedTime),getClr(normal,strobj)));
			}
			
			dxo.draw(Pint(5 ,105), dxoStrP(Pint(105,20),"Recieved at",getClr(normal,strobj)));
			if(isReceivedFlag){
				dxo.draw(Pint(115, 105), dxoStrP(Pint(105,20),hmr::time_to_hms(receivedTime),getClr(active,strobj)));
			}else{
				dxo.draw(Pint(115, 105),dxoStrP(Pint(105,20),hmr::time_to_hms(receivedTime),getClr(normal,strobj)));
			}
			
			dxo.draw(Pint(5 ,130),dxoStrP(Pint(105,20),"TimeOut at",getClr(normal,strobj)));
			if(isTimeOutedFlag){
				dxo.draw(Pint(115, 130),dxoStrP(Pint(105,20),hmr::time_to_hms(timeOutedTime),getClr(active,strobj)));
			}else{
				dxo.draw(Pint(115, 130),dxoStrP(Pint(105,20),hmr::time_to_hms(timeOutedTime),getClr(normal,strobj)));
			}
