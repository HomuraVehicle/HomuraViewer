#ifndef HMR_OPERATOR_INC
#define HMR_OPERATOR_INC 103
#
/*===hmrOperator===
MesasgeとComを制御して送受信タイミングをコントロールするクラス

hmrOperator:v1_03/130525 amby  
...
hmrOperator:v1_01/130511 amby
	Interval を制御するために　
	cFullRecvTimeSendOperatorに
	signal slot を追加する
...
hmrOperator:v1_00/130310 hmito
	cFullRecvOneSendOperatorを追加
...
namespace hmr{
	...
	private:
		...
		itfCom* pCom;
	public:
		cFullRecvOneSendOperator(itfMessage* pMessage_,itfCom* pCom_)
			:pMessage(pMessage_)
			,pCom(pCom_){
		}
		void operator()(void){
	...
	class cFullRecvTimeSendOperator{
	private:
		itfMessage* pMessage;
		itfCom* pCom;
	...
	public:
		cFullRecvTimeSendOperator(itfMessage* pMessage_,itfCom* pCom_,clock::duration Interval_,clock::duration TimeOut_, bool IsFullDuplex_, bool SyncFlag_)
			:pMessage(pMessage_)
			,pCom(pCom_)
			,IsFullDuplex(IsFullDuplex_)
			,Interval(Interval_)
	...
	public:
		cFHDxOperator(itfMessage* pMessage_,itfCom* pCom_, bool IsFullDuplex_,clock::duration Interval_,clock::duration TimeOut_)
			:pMessage(pMessage_)
			,pCom(pCom_)
			,IsSendActiveFlag(false)
			,IsFullDuplex(IsFullDuplex_)
