...
namespace hmr{
	namespace viewer{
		...
		public:
			...
			const dxRGB& getClr(status Sta_, object Obj_)const{
				...
				case butobj:
					...
					default:
						hmLib_throw(hmLib::memory_exceptions::memory_exception, "Invalid status.");
					}
				case strobj:
					switch(Sta_){
					case normal:
					...
					default:
						hmLib_throw(hmLib::numeric_exceptions::invalid_initialvalue, "Invalid status.");
					}
				default:
					hmLib_throw(hmLib::numeric_exceptions::invalid_initialvalue, "Invalid object.");
				}
			}
			const dxRGB& getTimeStrClr(clock::time_point Time_)const{
				...
				default:
					hmLib_throw(hmLib::numeric_exceptions::invalid_initialvalue, "Invalid Time Category");
				}
			}
			time getTime(clock::time_point Time_)const{
				if(clock::now() - Time_ < std::chrono::seconds(5))return present;
				else if(clock::now() - Time_ < std::chrono::seconds(15))return recent;
				else return past;
			}
		...
		public:
			...
			ALI::ALIGN Ali;
		public:
			dxoBUITimeStr(const dxosBUI* pBUI_, Pint Size_, const std::string& Str_, clock::time_point Time_, ALI::ALIGN Ali_ = ALI::center) :pBUI(pBUI_), hmoBox(Size_), Str(Str_), Time(Time_), Ali(Ali_){}
		public:
			int redraw(dxO& dxo)override{
