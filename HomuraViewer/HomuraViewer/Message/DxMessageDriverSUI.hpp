#ifndef HMR_VIEWER_MESSAGE_DXMESSAGEDRIVERSUI_INC
#define HMR_VIEWER_MESSAGE_DXMESSAGEDRIVERSUI_INC 100
...
/* ---- hmrDxMessageDriverSUI-----

hmrDxMessageDriver.hpp :v1_00/130615 amby
	DxMessageDriverSUIの製作開始
	実装
*/
...
namespace hmr{
	namespace viewer{
		namespace message{
			// 描画クラス static
			class dxosMessageDriverSUI :public dxosBUI{
			private:
				bool startFlag;
			...
			public:
				...
				int normal_draw(dxO& dxo)override{
					...
					// 自分自身を名乗って状態表示
					//dxo.draw(Pint(5,5),dxoStrP(Pint(70,20),"MessageDriver",getClr(normal,strobj)));
			...
			public:
				// constructor
				dxosMessageDriverSUI(void) :dxosBUI("MessageDriver", 80, 155), startFlag(false), timeOut(std::chrono::milliseconds(250)), timeInterval(std::chrono::milliseconds(250)), isFullDuplexFlag(true),
