#ifndef HMR_VIEWER_GPS_GPSKASHMIR_INC
#define HMR_VIEWER_GPS_GPSKASHMIR_INC
#
...
namespace hmr{
	namespace viewer{
		namespace gps{
			...
			private:
				void sendGPGGA(const data& Data_){
			...
			public:
				void slot_sendGPGGA(boost::signals2::signal<void(const data&, clock::time_point)>& signal_){
					SignalConnections(hmLib::signals::connect(signal_, [&](const data& Data_, clock::time_point t)->void{this->sendGPGGA(Data_); }));
				}
