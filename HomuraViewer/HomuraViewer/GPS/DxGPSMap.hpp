		...
		namespace gps{
			...
			private:
				...
				public:
					...
					void activate(pdouble GPSPos_){
						...
						GPSScale.x = a*cos(p) / sqrt(1 - e2*sin(p)*sin(p))*pi() / 180.;
						GPSScale.y = a*(1 - e2) / pow(1 - e2*sin(p)*sin(p), 1.5)*pi() / 180.;
					}
					//不活性化する
					void deactivate(){
						GPSRefPos.x = 0;
			...
			public:
				...
				*/
				...
				void slot_addNewData(boost::signals2::signal<void(const data&, clock::time_point)>& signal_){
					SignalConnections(hmLib::signals::connect(signal_,
						[&](const data& Data_, clock::time_point t)->void{
						lock Lock(this->Mx);
