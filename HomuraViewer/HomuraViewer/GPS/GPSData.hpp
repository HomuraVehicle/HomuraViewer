#ifndef HMR_VIEWER_GPS_DATA_INC
#define HMR_VIEWER_GPS_DATA_INC 101
#
		...
		namespace gps{
			...
			public:
				data() :
					...
					errFlag(false){}
				//コンストラクタ
				data(pdouble _Pos, double _Height, clock::time_point _Time, clock::time_point _UTC, bool _failFlag, bool _errFlag, int _UseGPS) :
					UseGPS(_UseGPS),
					Pos(_Pos),
					Height(_Height),
					Time(_Time),
					...
					errFlag(_errFlag){}
				//operatorオーバーライド
				bool operator<(const data& _dat)const{ return Time < _dat.Time; }
