					...
					for(auto pitr=DBegin - 1;pitr != DEnd;--pitr){
						...
						if(StrBegin == 0)// Header 部分の描画
						{
							...
							*/
							...
							//dxo.draw(Pint(120,DrawPosY+2),dxoStr(hmr::time_to_hms(pitr->second.BeginTime),Clr));
							dxo.draw(Pint(5,DrawPosY+2),dxoStr(hmr::time_to_hms(pitr->second.EndTime),Clr));

							//dxo.draw(Pint(25,DrawPosY + 22), dxoStr("Time End", Clr));
							//dxo.draw(Pint(120,DrawPosY + 22),dxoStr(hmr::time_to_hms(pitr->second.EndTime), Clr));
							dxo.draw(Pint(5,DrawPosY + 22),dxoStr(hmr::time_to_hms(pitr->second.BeginTime), Clr));
...
/*
	...
	protected:
		int fndraw(dxO& dxo){
			if(rIO.ioLogGate.Log.size()){
				dxo.draw(Pint(0,static_cast<int>(rIO.ioLogGate.Log.size()*40+20)),dxoNull());
				...
				for(int i=Acs().viewpos.y/40 ;i<=(Acs().size.y + Acs().viewpos.y)/40 && i<(int)rIO.ioLogGate.Log.size();i++){
					if(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getIO()){
						pClr=&InClr;
						if(dxo.draw(Pint(5,i*40+10),dxoBut(Pint(25,36),"I",*pClr))<0){
							InpSetter(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getStr());
						}
					}
					else{
						pClr=&OutClr;
						if(dxo.draw(Pint(5,i*40+10),dxoBut(Pint(25,36),"O",*pClr))<0){
							InpSetter(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getStr());
						}
					}

					hmDate tmpDate;
					tmpDate=hmDate(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getEndTime());
					dxo.draw(Pint(30,i*40+10),dxoStrP(Pint(90,20),hmstd::fStr("%02d%02d%02d.%03d",tmpDate.Hour,tmpDate.Minute,tmpDate.Second,tmpDate.mSecond),*pClr));
					tmpDate=hmDate(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getStartTime());
					dxo.draw(Pint(30,i*40+10+16),dxoStrP(Pint(90,20),hmstd::fStr("%02d%02d%02d.%03d",tmpDate.Hour,tmpDate.Minute,tmpDate.Second,tmpDate.mSecond),*pClr));

					dat=rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getStr();

					//端部保証処理
					dxo.draw(Pint(120+(int)dat.size()*24+24,i*40+10),dxoNull());
					for(int j=0;j<(int)dat.size();j++){
						//端部回避処理
						if(120+j*24+24<Acs().viewpos.x)continue;
						if(120+j*24>Acs().viewpos.x+Acs().size.x)break;
						//描画処理
						dxo.draw(Pint(120+j*24,i*40+10),dxoStrP(Pint(24,20),hmstd::fStr("%c",dat[j]),hm::sel(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getIO(),InClr,OutClr)));
						dxo.draw(Pint(120+j*24,i*40+10+16),dxoStrP(Pint(24,20),hmstd::fStr("%02x",(uchar)dat[j]),hm::sel(rIO.ioLogGate.Log[rIO.ioLogGate.Log.size()-i-1].getIO(),InClr,OutClr)));
					}
				}
			}
			return 0;
	...
	// Time out 系列　こいつらは個人的にはオペレータが管理したほうがよいと思う
	//size(80,50)
	...
	protected:
		virtual int fndraw(dxO &dxo){
			if(dxo.draw(Pint(5,2),dxoCheck(Pint(80,20),"TimeOut",20,Clr,hmrIO::iologType::ioTimeOutFlag))==1)hmrIO::iologType::ioTimeOutFlag=!hmrIO::iologType::ioTimeOutFlag;
			int val=static_cast<int>(hmrIO::iologType::ioTimeOut.time()*1000+hmrIO::iologType::ioTimeOut.msec());
			int ans=dxo.draw(Pint(0,27),dxoButLRIO(Pint(80,20),hmstd::fStr("%d",val),Clr,hmrIO::iologType::ioTimeOutFlag));
			if(ans==1){
				val*=2;
				if(val>4000)val=250;
	...
	protected:
		virtual int fndraw(dxO &dxo){
			dxo.draw(Pint(5,0),dxoStrP(Pint(60,45),"IO",Clr));
			if(dxo.draw(Pint(65,0),dxoCheck(Pint(80,20),"TimeOut",16,Clr,hmrIO::iologType::ioTimeOutFlag))==1)hmrIO::iologType::ioTimeOutFlag=!hmrIO::iologType::ioTimeOutFlag;
			int val=static_cast<int>(hmrIO::iologType::ioTimeOut.time()*1000+hmrIO::iologType::ioTimeOut.msec());
			int ans=dxo.draw(Pint(65,23),dxoButLRIO(Pint(80,20),hmstd::fStr("%d",val),Clr,hmrIO::iologType::ioTimeOutFlag));
			if(ans==1){
				val*=2;
				if(val>4000)val=250;
				hmrIO::iologType::ioTimeOut.set(val/1000,val%1000);
