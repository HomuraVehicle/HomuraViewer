#ifndef HMR_VIEWER_CSVFILEAGENT_INC
#define HMR_VIEWER_CSVFILEAGENT_INC 100
#
#include <iostream>
#include <string>
#include <fstream>
...
namespace hmr{
	namespace viewer{
		...
		public:
			...
			template<>
			class cCell<clock::time_point>{
			private:
				std::string Name;
				void regist_ownself(cCSVFileAgent& File_)const{
					File_.Cells.push_back(pair());
					File_.Cells.back().first = Name;
					hmLib::inquiries::connect(File_.Cells.back().second, [&](void)->std::string{return Inquiry.is_connect() ? time_to_hms(Inquiry()) : ""; });
				}
			public:
				cCell(const std::string& Name_) :Name(Name_){}
				hmLib::inquiries::inquiry<clock::time_point> Inquiry;
				friend class cCSVFileAgent;
			};
		private:
			std::ofstream ofs;
