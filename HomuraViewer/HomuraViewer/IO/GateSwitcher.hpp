#ifndef HMR_VIEWER_IO_GATESWITCHER_INC
#define HMR_VIEWER_IO_GATESWITCHER_INC 104
#
/*===hmrGateHab===
		...
		namespace io{
			...
			public:
				...
				struct cFomaSetting{
					...
					int Baudrate;
					std::string PhoneNo;
					cFomaSetting() :PortNo(0), Baudrate(9600), PhoneNo("00000000000"){}
				};
				struct cComSetting{
					int PortNo;
					int Baudrate;
			...
			public:
				...
				//Copy禁止
				cGateSwitcher(const my_type& My_) = delete;
				const my_type& operator=(const my_type& My_) = delete;
				//Move禁止（してもいいけど面倒）
				cGateSwitcher(my_type&& My_) = delete;
				const my_type& operator=(my_type&& My_) = delete;
			public:
