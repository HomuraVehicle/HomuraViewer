#ifndef HMR_VIEWER_IO_DXVMCSUI_INC
#define HMR_VIEWER_IO_DXVMCSUI_INC 100
		...
		namespace io{
			...
			public:
				...
				int extend_draw(dxO& dxo)override{
					...
					dxo.draw(Pint(5, 105), dxoStrP(Pint(105, 20), time_to_hms(forceResetSendTime), getClr(normal, strobj)));

					//dxo.draw(Pint(5,130), dxoStrP(Pint(105,20),"Last Force Recv End", getClr(normal,strobj)));
					dxo.draw(Pint(125, 105), dxoStrP(Pint(105, 20), time_to_hms(forceResetRecvTime), getClr(normal, strobj)));
