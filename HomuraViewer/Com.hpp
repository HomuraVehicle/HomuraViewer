...
//				clock::time_point LastRecvDatTime;
			...
			static hmLib_boolian existSendDat(void){
				// 念のためbuffチェック
				hmLib_assert( !pCom->SendBuf.empty(),hmLib::io_exceptions::io_exception, " VMCConnection::hmLib_boolian existSendDat -> SendBuf is empty !!" );
	...
	public:
		...
		//受信バッファ内からデータを取得する
		datum recv()override{
			hmLib_assert(!empty(),hmLib::io_exceptions::io_exception,"hmrCom::RecvBuf is empty");
		...
		void sync()override{
			hmLib_assert(can_sync(),hmLib::io_exceptions::io_exception,"hmrCom::RecvBuf have already been syncd");
		...
		//送信ストリームに送信バッファを反映する
		void flush()override{
			hmLib_assert(can_flush(),hmLib::io_exceptions::io_exception,"hmrCom::SendBuf cannot flush");
	...
	template<unsigned int VMC1_Connection_Ch_>
	datum cCom::VMCConnection<VMC1_Connection_Ch_>::TemporalSendDatum;
	template<unsigned int VMC1_Connection_Ch_>
	datum cCom::VMCConnection<VMC1_Connection_Ch_>::TemporalRecvDatum;
	template<unsigned int VMC1_Connection_Ch_>
